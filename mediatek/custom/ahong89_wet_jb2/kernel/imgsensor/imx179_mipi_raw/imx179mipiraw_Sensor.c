...
/**************ghong hhongyan add otp 20130802**************/
#define IMX179_OTP
#ifdef IMX179_OTP
kal_uint8 MID_Sunny=0x01; 
kal_uint8 LensID_Sunny=0x01; 
kal_uint8 VcmID_Sunny=0;

kal_uint8 RoverG_dec_base = 0x249; //R/G_Typical 
kal_uint8 BoverG_dec_base = 0x232; //B/G_Typical

bool ReadOTPIMX179();
bool IMX179_ReadWbFromOtp();
bool IMX179_ReadOtp(kal_uint16 tempbank,kal_uint16 address,kal_uint16* iBuffer,kal_uint16 buffersize);
void updateAWBIMX179(kal_uint16 RoverG_dec,kal_uint16 BoverG_dec,kal_uint16 GboverGr_dec);
...
/*******************ghong hhongyan otp start************************/
//read otp 
bool ReadOTPIMX179() 
{
 printk("ghong hhongyan enter ReadOTPIMX179 func\n");
 if(false == IMX179_ReadWbFromOtp()) 
 {printk("ghong hhongyan update_otp fail!\n");
  return FALSE; 
 }
 printk("ghong hhongyan update_otp sucess!\n");  
return TRUE; 
} 
...
bool IMX179_ReadWbFromOtp() 
{ 
 printk("ghong hhongyan enter IMX179_ReadWbFromOtp func\n");
 kal_uint16 awbGroupbank[] = {3,2,1}; 
 kal_uint16 address = 0x3404; 
 //USHORT MID=pVideo->pSW.MID; 
 //USHORT LensID=pVideo->pSW.LENSID; 
 //USHORT VcmID=pVideo->pSW.VCMID; 
 
 kal_uint16 r,g,b; 
 int i; 
 kal_uint16 Temp[42]={0}; 
 kal_uint16 temp3; 
 int index = -1; 
 for(i=0;i<3;i++)//表示 otp 烧录次数 
 { 
  IMX179MIPI_write_cmos_sensor(0x3380,0x00); 
  IMX179MIPI_write_cmos_sensor(0x3400,0x01); 
  IMX179MIPI_write_cmos_sensor(0x3402,awbGroupbank[i]); 
  temp3=IMX179MIPI_read_cmos_sensor(0x3404); 
  printk("ghong hhongyan check temp3=%d",temp3);
  if (temp3==1) //判断标志位 
  { 
   index = i; 
   break; 
  ...
  printk("ghong hhongyan check index=%d",index);
 if (index == -1)  
 { 
   // OTP中没有发现 AWB 数据 
  return false; 
 ...
 IMX179_ReadOtp(tempbank,address,Temp,42);
  
 if(Temp[6]!=MID_Sunny||Temp[7]!=LensID_Sunny||Temp[8]!=VcmID_Sunny) //一般只对MID进行检测
  { 
      //模组 ID、镜头 ID或者 VCM ID出错                 
      return false; 
  } 
 r = (Temp[15]<<8)|Temp[16]; 
 b = (Temp[17]<<8)|Temp[18]; 
...
bool IMX179_ReadOtp(kal_uint16 tempbank,kal_uint16 address,kal_uint16* iBuffer,kal_uint16 buffersize) 
{
 printk("ghong hhongyan enter IMX179_ReadOtp func\n");
 int i; 
 kal_uint16 reVal; 
 IMX179MIPI_write_cmos_sensor(0x3380,0x00); 
 IMX179MIPI_write_cmos_sensor(0x3400, 0x01); 
 IMX179MIPI_write_cmos_sensor(0x3402, tempbank);  
 for(i=0;i<buffersize;i++) 
 { 
  reVal = IMX179MIPI_read_cmos_sensor(address+i); 
  *(iBuffer+i) = reVal; 
  printk("otp_data[%d] = %d\n",i,reVal);
 } 
 return true; 
...
void updateAWBIMX179(kal_uint16 RoverG_dec,kal_uint16 BoverG_dec,kal_uint16 GboverGr_dec)
{ 
    printk("ghong hhongyan enter updateAWBIMX179 func\n");
	...
	IMX179MIPI_write_cmos_sensor(0x020F,G_test_L8); 
	    
	IMX179MIPI_write_cmos_sensor(0x0211,R_test_L8); 
	    
	IMX179MIPI_write_cmos_sensor(0x0213,B_test_L8); 
	    
	IMX179MIPI_write_cmos_sensor(0x0215,G_test_L8);    
}

/*****************ghong hhongyan otp end****************************/
...
static void IMX179MIPI_Sensor_Init(void)
{
	...
	#ifdef IMX179_OTP
	printk("ghong hhongyan update_otp start\n");
	ReadOTPIMX179();	//ghong hhongyan add otp 20130802
	#endif
...
UINT32 IMX179MIPIOpen(void)
{
	...
	printk("[IMX179MIPI]test i2c transfer\n");
	IMX179MIPI_write_cmos_sensor(0x41C0, 0x01);
	int testreg=0;	
	testreg=IMX179MIPI_read_cmos_sensor(0x41C0);
	printk("testreg =%d\n",testreg);
