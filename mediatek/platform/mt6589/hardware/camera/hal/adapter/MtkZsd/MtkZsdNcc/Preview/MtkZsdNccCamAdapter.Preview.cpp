...
onHandleStartPreview()
{
    ...
    CPTLog(Event_Hal_Adapter_MtkZsdPreview_start, CPTFlagStart);
    if ( ! mpResourceLock->SetMode(ResourceLock::eMTKZSDNCC_PRV) )
    {
    ...
    //
    if ( ! mpResourceLock->Lock(ResourceLock::eMTKZSDNCC_PRV) )
    {
    ...
    //
    CPTLog(Event_Hal_Adapter_MtkZsdPreview_start_init, CPTFlagStart);
    if ( ! mpPreviewCmdQueThread->postCommand(PrvCmdCookie::eStart, PrvCmdCookie::eSemAfter) )
    {
        MY_LOGE("StartPreview stage 1 (start): fail");
        goto lbExit;
    }
    CPTLog(Event_Hal_Adapter_MtkZsdPreview_start_init, CPTFlagEnd);
    ...
    //
    CPTLog(Event_Hal_Adapter_MtkZsdPreview_start, CPTFlagEnd);
    return OK;
   //
lbExit:
    return INVALID_OPERATION;
...
onHandleStopPreview()
{
    ...
    CPTLog(Event_Hal_Adapter_MtkZsdPreview_stop, CPTFlagStart);
    ...
    //
    if ( ! mpResourceLock->Unlock(ResourceLock::eMTKZSDNCC_PRV) )
    {
        CAM_LOGE("Resource Lock fail");
        return INVALID_OPERATION;
    }
    ...
    CPTLog(Event_Hal_Adapter_MtkZsdPreview_stop, CPTFlagEnd);
...
onHandlePreCapture()
{
    ...
    CPTLog(Event_Hal_Adapter_MtkZsdPreview_precap, CPTFlagStart);
    ...
    CPTLog(Event_Hal_Adapter_MtkZsdPreview_precap, CPTFlagEnd);
