...
onHandleStartPreview()
{
    ...
    if ( ! mpResourceLock->SetMode(ResourceLock::eMTKZSDCC_PRV) )
    {
    ...
    //
    if ( ! mpResourceLock->Lock(ResourceLock::eMTKZSDCC_PRV) )
    {
...
onHandleStopPreview()
{
    ...
    //
    if ( ! mpResourceLock->Unlock(ResourceLock::eMTKZSDCC_PRV) )
    {
        CAM_LOGE("Resource Lock fail");
        return INVALID_OPERATION;
    }
