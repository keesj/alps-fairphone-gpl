...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::doPvAE(MVOID *pAEStatBuf,MVOID *pWBgResult)
{
strAEInput rAEInput;
...
MUINT32 u4DFpsThres = 0;
    ...
    if(m_bEnableAE) {
        if(m_pIAeAlgo != NULL) {
			m_pIAeAlgo->SetWBGResult(pWBgResult);
            m_pIAeAlgo->setAESatisticBufferAddr(pAEStatBuf);
			if(g_bIsAutoFlare==TRUE)
			{
...
void AeMgr::setRestore(int frm)
{
	...
	{
		XLOGD("af sync support line=%d",__LINE__);
		setExp(g_rExp);
...
#else
	{
		XLOGD("af sync NOT support line=%d",__LINE__);
		setExp(g_rExp);
	}
#endif
...
//		AAASensorMgr::getInstance().getSensorSyncinfo(&i4SutterDelay, &i4SensorGainDelay, &i4IspGainDelay);
	...
	//		MY_LOG("setRestore delay %d %d %d\n", i4SutterDelay, i4SensorGainDelay, i4IspGainDelay );



//		MY_LOG("setRestore line=%d frm=%d, frmRate=%d\n", __LINE__, frm, frmRate);
...
MRESULT AeMgr::doCapFlare(MVOID *pAEStatBuf,MVOID *pWBgResult,MBOOL bIsStrobe)
{
strAEInput rAEInput;
strAEOutput rAEOutput;
AE_MODE_CFG_T a_rCaptureInfo;

    if(g_rAEInitInput.rAENVRAM.rCCTConfig.bEnableCaptureThres==FALSE ||
         g_rAEInitInput.rAENVRAM.rCCTConfig.bEnableVideoThres==FALSE ||
         g_rAEInitInput.rAENVRAM.rCCTConfig.bEnableStrobeThres==FALSE  ||
        g_bIsAutoFlare==FALSE  )
        return S_AE_OK;
...
//    a_strFrameInfo.i4LightValue_x10 = (m_BVvalue + 50);
    if(m_pIAeAlgo != NULL) {
        m_pIAeAlgo->getPlineTable(m_CurrentPreviewTable, m_CurrentCaptureTable);
//        MY_LOG("[getCurrentPlineTable] PreId:%d CapId:%d\n", m_CurrentPreviewTable.eID, m_CurrentCaptureTable.eID);
        a_strFrameInfo.i4LightValue_x10 = (m_CurrentPreviewTable.i4MaxBV-g_rAEInitInput.rAENVRAM.rCCTConfig.i4BVOffset) -(MINT32)m_i4AEidxNext + 50;   // AE EV value
    } else {
        MY_LOG("The AE algo class is NULL (37)\n");
    }
