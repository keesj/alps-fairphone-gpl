...
#if 0
//#ifdef MTK_PRECAPTURE_AF_SUPPORT
	XLOGD("pre-vsync MTK_PRECAPTURE_AF_SUPPORT line=%d",__LINE__);
	if(g_bCheckAF==1)
	{
		XLOGD("pre-vsync g_bCheckAF=1 line=%d",__LINE__);
		int isDoAF2;
		isDoAF2 = cust_isNeedDoPrecapAF(
		...
		if(isDoAF2==1)
		{
			XLOGD("pre-vsync isDoAF2=1 line=%d",__LINE__);
			...
			g_bCheckAF=0;
			XLOGD("pre-vsync return line=%d",__LINE__);
			return S_3A_OK;
		}
	}
	XLOGD("pre-vsync no-AF line=%d",__LINE__);
#endif
