...
//#endif
	...
	else 
	{
		...
		cur_eint_state = EINT_PIN_PLUG_IN;
	        /*----------ghong tyang add for auto detect begin------------*/
	        mt_set_gpio_mode(25, GPIO_MODE_GPIO);
	        mt_set_gpio_dir(25, GPIO_DIR_OUT);
	        mt_set_gpio_out(25, 0);
			  	mdelay(1);
	        mt_set_gpio_out(25, 1);
                /*----------ghong tyang add for auto detect end------------*/
#ifdef ACCDET_LOW_POWER
	...
	switch(accdet_status)
    {
        case PLUG_OUT:
            ...
            if(current_status == 0)
            {
				...
				mutex_lock(&accdet_eint_irq_sync_mutex);	
				if(1 == eint_accdet_sync_flag) {
					cable_type = HEADSET_NO_MIC;		//ghong hhongyan modify 20131114
					accdet_status = HOOK_SWITCH;
				  }else {
					ACCDET_DEBUG("[Accdet] Headset has plugged out\n");
				  }
