...
void pchr_turn_on_charging (void)
{
    ...
    else
    {
        ...
        {            
            ...
            upmu_set_rg_hwcv_en(1);                 // RG_HWCV_EN
            ...
            {
                upmu_set_rg_vbat_cv_vth(0x17); // VBAT_CV_VTH,4V low temp
            }
    ...
    if (Enable_BATDRV_LOG == 1) {
        printf("[BATTERY] Reg[0x%x]=0x%x, Reg[0x%x]=0x%x(VBAT_CV_VTH), Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\r\n", 			
...
void dump_chr_register(void)
{
    ...
    for (i=CHR_CON0;i<=CHR_CON29;i++)
    {
        ...
        printf("(%d)Reg[0x%x]=0x%x", ret, i, reg_val);
...
{    
    ...
    if ((upmu_is_chr_det() == KAL_TRUE))    
    {
        ...
        while (1) 
        {
            ...
            printf("BAT CHK STA: rtc_boot_check = 0x%x, meta_mode_check = 0x%x, pwrkey_ready = 0x%x\n", 
                                 rtc_boot_check(true), meta_mode_check(), pwrkey_ready); 
            printf("BAT CHK STA: mtk_wdt_boot_check = 0x%x, g_boot_arg->boot_reason = 0x%x, g_boot_menu = 0x%x\n", 
                                 mtk_wdt_boot_check(), g_boot_arg->boot_reason,  g_boot_menu);
                ...
                || mtk_wdt_boot_check()==WDT_BY_PASS_PWK_REBOOT || g_boot_arg->boot_reason==BR_TOOL_BY_PASS_PWK || g_boot_menu==true )
            {
                ...
                {    
                    ...
                    //upmu_set_baton_tdet_en(0); //sw workaround: 26M off
                    ...
                    //upmu_set_baton_tdet_en(1);  //sw workaround: 26M off
