...
/* MSDC_PATCH_BIT mask */
//#define CKGEN_RX_SDClKO_SEL     (0x1  << 0)     /*This bit removed on MT6589/MT6585*/ 
#define MSDC_PATCH_BIT_ODDSUPP    (0x1  <<  1)     /* RW */
//#define MSDC_PATCH_BIT_CKGEN_CK (0x1  << 6)     /* This bit removed on MT6589/MT6585 (Only use internel clock) */
...
#define MSDC_SET_FIELD(reg,field,val) __msdc_set_field(reg,field,val)
...
static inline void msdc_timer_init(void)
{
    ...
    DRV_WriteReg32(APMCU_GPTIMER_BASE + 0x30,0x0);
    DRV_WriteReg32(APMCU_GPTIMER_BASE + 0x30,0x2);

    DRV_WriteReg32(APMCU_GPTIMER_BASE + 0x38,0);
    DRV_WriteReg32(APMCU_GPTIMER_BASE + 0x3C,32768);

    /* 32678HZ RTC free run */
    DRV_WriteReg32(APMCU_GPTIMER_BASE + 0x34,0x30);
    DRV_WriteReg32(APMCU_GPTIMER_BASE + 0x30,0x32);

}
static inline void msdc_timer_start(void)
{
    *(volatile unsigned int*)(APMCU_GPTIMER_BASE + 0x30) |= (1 << 0);
}
...
{
    *(volatile unsigned int*)(APMCU_GPTIMER_BASE + 0x30) &= ~(1 << 0); /* stop  */
    *(volatile unsigned int*)(APMCU_GPTIMER_BASE + 0x30) |= (1 << 1);  /* clear */
}
static inline unsigned int msdc_timer_get_count(void)
{
    return DRV_Reg32(APMCU_GPTIMER_BASE + 0x38);
}
#else
