...
static u8 *dat_buf = (u8 *)COMMON_BUFFER_ADDR;		//(0x80000);
static u8 *oob_buf = (u8 *)(COMMON_BUFFER_ADDR+4096);		//(0x80000 + 4096);
...
MSG(INIT,  "[%s]: bmt_index=0x%x-- ", __FUNCTION__, bmt_index);
 ...
 memset(COMMON_BUFFER_ADDR,0xAA,4096);
 memset(oob_buf,0xAA,128);
        if (!nand_read_page_bmt(PAGE_ADDR(bmt_index), COMMON_BUFFER_ADDR, oob_buf))
        //if (!nand_read_page_bmt(0x3FFC0, COMMON_BUFFER_ADDR, oob_buf))
        {
            MSG(INIT, "Error found when read block %d\n", bmt_index);
            continue;
        }

        if (!match_bmt_signature(COMMON_BUFFER_ADDR, oob_buf))
        {
...
#endif        
        memcpy(&phys_table, COMMON_BUFFER_ADDR + MAIN_SIGNATURE_OFFSET, sizeof(phys_table));
